...
//extern const int MAXMODES = 10;
	...
	void TaskData::set_active(int i){
		//fprintf(stderr,"%d setting %d at %p to passive\n",index,i,(void*)&passive[i]);
		active[i]=true;
	}
	...
	void TaskData::set_current_mode(int new_mode, bool disable)
	{
		...
		{
			fprintf(stderr, "Error: Task %d was told to go to invalid mode %d. Ignoring.",get_index(), new_mode);
		}
	}
