...
void scheduler_task()
{
	...
	//fprintf(stderr, "scheduler past second barrier\n");
	if (ret_val != 0)
	{
		fprintf(stderr, "ERROR: Barrier error for scheduling task.\n");
		kill(0, SIGTERM);
	}
...
int main(int argc, char *argv[])
{
	...
	if (schedule_ret_val == -1)
	{
		fprintf(stderr, "ERROR: Cannot find schedule file: %s\n", schedule_filename.c_str());
	...
	{
		fprintf(stderr, "ERROR: First line of .rtps file error.\n Format: <taskset schedulability>.\n");
		return RT_GOMP_CLUSTERING_LAUNCHER_FILE_OPEN_ERROR;
	}
	if(!schedulable)
	{
		fprintf(stderr, "ERROR: Taskset deemed not schedulable by .rtps file. Exiting.\n");
		return RT_GOMP_CLUSTERING_LAUNCHER_FILE_OPEN_ERROR;
	}
	...
	{
		fprintf(stderr, "ERROR: Second line of .rtps file error.\n Format: <number of tasks> <s to run> <ns to run>.\n");
		return RT_GOMP_CLUSTERING_LAUNCHER_FILE_OPEN_ERROR;
	}
	...
	{
		fprintf(stderr, "ERROR: Found %u tasks and expected %u.\n",num_lines/2,num_tasks);
	...
	//end_time.tv_sec = start_time.tv_sec + secs_to_run;
	...
	{		
		if (std::getline(ifs, task_command_line))
		{
			...
			vector <timespec> work;
			vector <timespec> span;
			vector <timespec> period;
			int num_modes;
			...
			{       
				...
				if((task_timing_stream >> elasticity) && (task_timing_stream >> num_modes))
				{
					...
					if(num_modes <= 0)
					{
						fprintf(stderr, "ERROR: Task %d timing data. At least 1 mode of operation is required. Found %d.\n", t, num_modes);
						kill(0, SIGTERM);
						return RT_GOMP_CLUSTERING_LAUNCHER_FILE_PARSE_ERROR;
					}
				}
				...
				{
					fprintf(stderr, "ERROR: Task %d timing data. Mal-formed elasticity value and/or number of modes.\n", t);
					kill(0, SIGTERM);
					return RT_GOMP_CLUSTERING_LAUNCHER_FILE_PARSE_ERROR;
				}
				...
				for(int i=0; i<num_modes; i++)
				{
					...
					{
						fprintf(stderr, "ERROR: Task %d timing data. Mal-formed work, span, or period in mode %d.\n", t, i);
					}
				}
			...
			{
				fprintf(stderr, "ERROR: Too few timing parameters were provided for task %s.\n", program_name.c_str());
				kill(0, SIGTERM);
				return RT_GOMP_CLUSTERING_LAUNCHER_FILE_PARSE_ERROR;
			}
		...
		{
			fprintf(stderr, "ERROR: Please include at least one task.\n");
			kill(0, SIGTERM);
			return RT_GOMP_CLUSTERING_LAUNCHER_FILE_PARSE_ERROR;
		}
	}
	...
	//fprintf(stderr, "Scheduler reached scheduling barrier.");	
	ret_val = await_single_use_barrier(barrier_name2.c_str());
	if (ret_val != 0)
	{
		fprintf(stderr, "ERROR: Barrier error for scheduling task \n");
		kill(0, SIGTERM);
